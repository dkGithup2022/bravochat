...
-- Turn A 이벤트
INSERT INTO turn_events (turn_id, seq, type, content, is_deleted, created_at, updated_at)
SELECT t.id, 1, 'USER_MESSAGE', '안녕하세요', FALSE, t.created_at, t.created_at
FROM turns t WHERE t.user_id = 1 AND t.created_at = TIMESTAMP '2026-07-20 01:00:00'
  AND NOT EXISTS (SELECT 1 FROM turn_events e WHERE e.turn_id = t.id AND e.seq = 1);
INSERT INTO turn_events (turn_id, seq, type, content, is_deleted, created_at, updated_at)
SELECT t.id, 2, 'ASSISTANT_MESSAGE', '안녕하세요. 무엇을 도와드릴까요?', FALSE, t.created_at, t.created_at
FROM turns t WHERE t.user_id = 1 AND t.created_at = TIMESTAMP '2026-07-20 01:00:00'
  AND NOT EXISTS (SELECT 1 FROM turn_events e WHERE e.turn_id = t.id AND e.seq = 2);

-- Turn B 이벤트 (USER → TOOL_CALL → TOOL_RESULT → ASSISTANT)
INSERT INTO turn_events (turn_id, seq, type, content, is_deleted, created_at, updated_at)
SELECT t.id, 1, 'USER_MESSAGE', '오늘 서울 날씨 알려줘', FALSE, t.created_at, t.created_at
FROM turns t WHERE t.user_id = 1 AND t.created_at = TIMESTAMP '2026-07-20 01:05:00'
  AND NOT EXISTS (SELECT 1 FROM turn_events e WHERE e.turn_id = t.id AND e.seq = 1);
INSERT INTO turn_events (turn_id, seq, type, content, tool_name, tool_call_id, is_deleted, created_at, updated_at)
SELECT t.id, 2, 'TOOL_CALL', '{"city":"서울"}', 'get_weather', 'call_1', FALSE, t.created_at, t.created_at
FROM turns t WHERE t.user_id = 1 AND t.created_at = TIMESTAMP '2026-07-20 01:05:00'
  AND NOT EXISTS (SELECT 1 FROM turn_events e WHERE e.turn_id = t.id AND e.seq = 2);
INSERT INTO turn_events (turn_id, seq, type, content, tool_call_id, is_deleted, created_at, updated_at)
SELECT t.id, 3, 'TOOL_RESULT', '{"temp":25,"sky":"맑음"}', 'call_1', FALSE, t.created_at, t.created_at
FROM turns t WHERE t.user_id = 1 AND t.created_at = TIMESTAMP '2026-07-20 01:05:00'
  AND NOT EXISTS (SELECT 1 FROM turn_events e WHERE e.turn_id = t.id AND e.seq = 3);
INSERT INTO turn_events (turn_id, seq, type, content, is_deleted, created_at, updated_at)
SELECT t.id, 4, 'ASSISTANT_MESSAGE', '오늘 서울은 맑고 25도입니다.', FALSE, t.created_at, t.created_at
FROM turns t WHERE t.user_id = 1 AND t.created_at = TIMESTAMP '2026-07-20 01:05:00'
  AND NOT EXISTS (SELECT 1 FROM turn_events e WHERE e.turn_id = t.id AND e.seq = 4);

-- Turn C 이벤트
INSERT INTO turn_events (turn_id, seq, type, content, is_deleted, created_at, updated_at)
SELECT t.id, 1, 'USER_MESSAGE', '고마워', FALSE, t.created_at, t.created_at
FROM turns t WHERE t.user_id = 1 AND t.created_at = TIMESTAMP '2026-07-20 01:10:00'
  AND NOT EXISTS (SELECT 1 FROM turn_events e WHERE e.turn_id = t.id AND e.seq = 1);
INSERT INTO turn_events (turn_id, seq, type, content, is_deleted, created_at, updated_at)
SELECT t.id, 2, 'ASSISTANT_MESSAGE', '천만에요!', FALSE, t.created_at, t.created_at
FROM turns t WHERE t.user_id = 1 AND t.created_at = TIMESTAMP '2026-07-20 01:10:00'
  AND NOT EXISTS (SELECT 1 FROM turn_events e WHERE e.turn_id = t.id AND e.seq = 2);

-- Turn D 이벤트 (PROCESSING — USER만, 결과에서 제외)
INSERT INTO turn_events (turn_id, seq, type, content, is_deleted, created_at, updated_at)
SELECT t.id, 1, 'USER_MESSAGE', '처리 중인 질문', FALSE, t.created_at, t.created_at
