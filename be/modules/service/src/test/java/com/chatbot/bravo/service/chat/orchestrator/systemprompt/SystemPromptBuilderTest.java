...
class SystemPromptBuilderTest {
    ...
    void 도구있음_4섹션_순서와_캐시경계() {
        ...
        assertThat(s).extracting(PromptSection::name)
                .containsExactly("core", "persona", "tool_guidance", "style");
        assertThat(s).extracting(PromptSection::cacheable)
                .containsExactly(true, true, false, false); // cacheable 은 앞쪽에 연속
    }
    ...
    void 부재_도구없으면_tool_guidance_생략() {
        ...
        assertThat(s).extracting(PromptSection::name)
                .containsExactly("core", "persona", "style"); // tool_guidance 부재
    }
    ...
    void 재현_주문봇_샘플_조립() {
        ...
        assertThat(prompt)
                .startsWith("당신은 ShopKr의 고객 지원 어시스턴트 '주문봇'입니다.")
